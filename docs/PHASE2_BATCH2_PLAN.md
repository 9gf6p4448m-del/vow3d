...
### R12 r3 覆審結果（2026-09-19；報告 `vow-toolchain/REVIEW-p2b2-r3.md`，標的 `0d53e20`）
- 三態：真的修好 7／表面修好 0／沒修到 0；新 finding：CRITICAL 0／HIGH 0／MEDIUM 1／LOW 1。凍結判準未被移動（刪除行 7 行，全屬 V11-h 授權、註解或加嚴）。這是第 3 輪＝上限，不再開第 4 輪。
- **R3-1（MEDIUM，未修，待使用者裁定）**：替代點快取的命中條件只有「目的地格＋格點版本」（`HeroLocomotion.cs:278-284`），但 R1a 是用連續的目的地座標選點——走不到的目標在同一格內移動時，快取的替代點可能不是當下的 R1a 答案（覆審實跑：落差 1.803m、在牆的另一側）。v0.4.1 場上沒有會動的追擊目標，玩家踩不到；**批 3 加入會動的敵人之前必修**（方向：快取 key 改含目的地的連續座標）。
- R3-2（LOW，記錄不修）：V11-b 的 300 盤沒取樣到「目的地貼邊界圈／英雄站 Blocked 格」；覆審以新舊碼對跑 6000 盤（含這兩類）0 筆不一致，屬涵蓋缺口。

### R13 批 3 前置：R3-1 的凍結驗收條件（2026-09-19 動手前訂定；已知良好狀態＝main `59af5bf`；使用者 09-19「先下一步」→ 照 R12 的建議列為批 3 前必修）
**V13-a（PlayMode，r3 覆審 R3-1 的重現）** 追一個走不到的木樁（站在某面牆的外擴區內），英雄抵達替代點後，把木樁在**同一格內**平移到一個「R1a 答案會換格」的位置（測試要斷言平移前後 `TryWorldToCell` 同格，且用全新的 `GridNavigator.ResolveGoal`（同一份格點、英雄當下位置）算出的新答案與舊答案不同格——這一條是活性，幾何挑錯時要紅而不是無聲通過）。平移後 0.35 秒內：`NavMeshAgent.destination` 與「當下 R1a 答案」的平面距離 ≤ 0.05m。另留一條寫死座標的錨點斷言（新答案的格心座標寫成常數）。紅燈條件：`59af5bf` 的碼（目的地凍在舊替代點；r3 實跑落差 1.803m）。
**V13-b（不得退回 M3）** V11-c（靜止目標 3 秒 `BuildCount` ≤4、最後 1 秒＝0）、V11-d①②③、V11-j 原封不動全綠。
**V13-c（回歸）** `verify.sh` ALL PASS、突變 56/56、Unity EditMode／PlayMode failed=0；`Assets/Tests` 既有測試的門檻、期望值、容差零改動；§3 六檔與 asmdef 零改動。
