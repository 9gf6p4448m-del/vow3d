...
| **擴散火浪**（風＋火） | `FIRE` 打空地生燃燒區 → 走到能把它罩進扇形的位置 → `WIND` | 扇形預警亮起、燃燒區消失、扇形內的目標吃 60 傷害＋一次頓挫震屏 |
...
- HUD 面板已經在臨界：整塊面板需要 `Screen.height / scale ≥ 503.2`，WebGL 橫式約需 CSS 視窗高 ≥302px 才放得下最後一列（§9 M6）。真正的閘是下面那一節的線上實測。
...
- **線上實測與四顆鈕的 CSS 座標：（待主對話部署後以 Playwright 實測填入）** —— 需要填的欄位：`git log origin/gh-pages -1` 的時間戳與 SHA、線上首頁版本列、四顆鈕在直式（390×844）與橫式（844×390）下的 CSS 座標與可見性（§9 M6 的閘）、三個反應各一輪的截圖路徑、console 有無 error。
