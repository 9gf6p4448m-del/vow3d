...
- **點牆腳／走不到的地方**：英雄走到「最近可達點」停下（不發呆、不頂牆）。兩側差不多近時，優先停在英雄所在的這一側；只有明顯點在牆的另一面才會繞過去。
- **牆壓到英雄**（例：打爆測試牆後站在原地等它 6 秒後重生）：牆照立，英雄被推到最近的空格。
...
- 邊界圈找不到場景的邊界 Collider 時會無聲失效（目前靠測試擋）；Unity 端的零配置量測窗口還沒涵蓋「點牆腳」這條路（dotnet 端量過是 0 bytes）。
