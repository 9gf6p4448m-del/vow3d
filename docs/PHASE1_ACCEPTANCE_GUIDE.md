...
- **按住往外拖**＝自由封路：地面出現半透明虛影，拇指方向＝石牆相對英雄的方位，拉得越遠牆越遠（2m～8m）；鬆手成牆。
- **取消**：滑回按下的位置放手，或往上滑進按鈕正上方的紅色取消區（拖曳中才顯示，一路延伸到螢幕頂）放手。取消不吃冷卻。
...
按鈕版面（`Assets/Scripts/Input/RuneButtonLayout.cs`）：直徑 16mm、離邊 4mm、取消區下緣＝按鈕上緣＋14mm（拉滿）＋14mm（超拉餘裕）。
...
3. **拖曳原點＝手指按下的位置**（浮動原點，與微輪盤一致）；「滑回按鈕中心」＝回到距按下位置 3.5mm 內。
...
只有真人能判斷、尚未驗證的：符印鈕在真機上的大小與位置順不順手；0.2 秒／3.5mm／7.5mm 三個輕點判定值（症狀：「點了符印鈕卻沒反應」或「想在正前方立牆卻立歪在旁邊 2m」）；8 秒冷卻與 2～8m 距離的手感；取消區紅柱的觀感；開 NET delay 時虛影與實牆的時間差。
