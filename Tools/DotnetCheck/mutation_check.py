...
MUTATIONS = [
    ...
    ("U11", "取消區只是一條帶子（拉穿之後又變回成牆）", INPUT + "RuneButtonLayout.cs",
     "layout.CancelZone = new ScreenRegion(layout.Button.XMin, cancelBottom, layout.Button.XMax, screenHeight);",
     "layout.CancelZone = new ScreenRegion(layout.Button.XMin, cancelBottom, layout.Button.XMax, cancelBottom + 12f * pixelsPerMillimeter);",
     "FlingingFarAboveTheButton_StillCancels"),
    ("U12", "矮螢幕上取消區被擠到消失", INPUT + "RuneButtonLayout.cs",
     "if (cancelBottom > highestAllowedBottom) cancelBottom = highestAllowedBottom;", "",
     "Layout_OnAShortCanvas_KeepsAUsableCancelZone"),
