...
MUTATIONS = [
    ...
    ('C10', '在倒地當下決定復活點（藍方一律回家）', LOGIC + 'CaptureMatchLogic.cs',
     ...
     '                    _bluePendingRespawnX = _spec.MotherRespawnX(BlueFactionId, 0);\n                    _bluePendingRespawnZ = _spec.MotherRespawnZ(BlueFactionId, 0);\n',
     'VA17_RespawnLocation_HomeUnlessTheBaseWasCaptured'),
    ...
    ('C12', '屍體算在場（拿掉倒地強制 -1）', LOGIC + 'CaptureMatchLogic.cs',
     ...
     '            int blueCircle = _spec.CircleAt(heroX, heroZ, _tuning.CircleRadius);\n            int redCircle = _spec.CircleAt(opponentX, opponentZ, _tuning.CircleRadius);\n',
    ...
    # ── v0.9.0 步驟 A（V090_ENCIRCLE_PLAN.md §3-A V9-A20，2026-09-25 凍結） ──
    ('N1', '不跑 BFS（拿掉 ③b）', LOGIC + 'CaptureMatchLogic.cs',
     '            if (blueFlipped != -1 || redFlipped != -1) ProcessEncircle(blueFlipped, redFlipped);   // ③b 包夾 BFS 與狂怒判定\n',
     '',
     'V9A07_CutOffTile_IsNeutralizedBeforeScoring_OnTheFlipTick'),
    ('N2', 'BFS 排在 ⑤ 計分之後', LOGIC + 'CaptureMatchLogic.cs',
     '            int blueCircleOwnerBefore = blueCircle == -1 ? -1 : _ownership[blueCircle];\n            int redCircleOwnerBefore = redCircle == -1 ? -1 : _ownership[redCircle];\n\n            ProcessChannels(blueCircle, redCircle, dt);   // ③ 引導／爭奪／翻塊\n\n            int blueFlipped = blueCircle != -1 && blueCircleOwnerBefore != BlueFactionId && _ownership[blueCircle] == BlueFactionId ? blueCircle : -1;\n            int redFlipped = redCircle != -1 && redCircleOwnerBefore != RedFactionId && _ownership[redCircle] == RedFactionId ? redCircle : -1;\n            if (blueFlipped != -1 || redFlipped != -1) ProcessEncircle(blueFlipped, redFlipped);   // ③b 包夾 BFS 與狂怒判定\n\n            // ④ 必須排在 ③ 之後：復活地點看得到本 tick 的翻塊（E20；把它搬到 ③ 之前就是突變 N13）。\n            ProcessKnockouts(dt);   // ④ 倒地倒數與復活地點\n\n            ProcessScoring(dt);   // ⑤ 計分時鐘 ⑥ 勝負\n',
     '            int blueCircleOwnerBefore = blueCircle == -1 ? -1 : _ownership[blueCircle];\n            int redCircleOwnerBefore = redCircle == -1 ? -1 : _ownership[redCircle];\n\n            ProcessChannels(blueCircle, redCircle, dt);   // ③ 引導／爭奪／翻塊\n\n            int blueFlipped = blueCircle != -1 && blueCircleOwnerBefore != BlueFactionId && _ownership[blueCircle] == BlueFactionId ? blueCircle : -1;\n            int redFlipped = redCircle != -1 && redCircleOwnerBefore != RedFactionId && _ownership[redCircle] == RedFactionId ? redCircle : -1;\n\n            // ④ 必須排在 ③ 之後：復活地點看得到本 tick 的翻塊（E20；把它搬到 ③ 之前就是突變 N13）。\n            ProcessKnockouts(dt);   // ④ 倒地倒數與復活地點\n\n            ProcessScoring(dt);   // ⑤ 計分時鐘 ⑥ 勝負\n            if (blueFlipped != -1 || redFlipped != -1) ProcessEncircle(blueFlipped, redFlipped);   // ③b 包夾 BFS 與狂怒判定\n',
     'V9A07_CutOffTile_IsNeutralizedBeforeScoring_OnTheFlipTick'),
    ('N3', 'BFS 只對翻塊方跑', LOGIC + 'CaptureMatchLogic.cs',
     '            EncircleSide(BlueFactionId, blueFlipped, BlueScore, leader);\n            EncircleSide(RedFactionId, redFlipped, RedScore, leader);\n',
     '            if (blueFlipped != -1) EncircleSide(BlueFactionId, blueFlipped, BlueScore, leader);\n            if (redFlipped != -1) EncircleSide(RedFactionId, redFlipped, RedScore, leader);\n',
     'V9A07_CutOffTile_IsNeutralizedBeforeScoring_OnTheFlipTick'),
    ('N4', '剛翻下的塊豁免 BFS', LOGIC + 'CaptureMatchLogic.cs',
     '                if (_ownership[i] == side && !_bfsReached[i])\n',
     '                if (_ownership[i] == side && !_bfsReached[i] && i != justFlipped)\n',
     'V9A06_IslandFlip_IsNeutralizedOnTheSameTick_ControlAdjacentToMotherStays'),
    ('N5', '根包含對方持有的己方母板塊', LOGIC + 'CaptureMatchLogic.cs',
     '                int m = _spec.MotherTile(side, k);\n                if (_ownership[m] == side)\n',
     '                int m = _spec.MotherTile(side, k);\n                if (true)\n',
     'V9A10_AllMothersLost_NeutralizesEverything_AndRespawnGoesToTheEdge_IncludingOnTheSameTick'),
    ('N6', '落後判定用 >=', LOGIC + 'CaptureMatchLogic.cs',
     '_tuning.RageDeficitDenominator * (leader - ownScore) > _tuning.RageDeficitNumerator * leader)',
     '_tuning.RageDeficitDenominator * (leader - ownScore) >= _tuning.RageDeficitNumerator * leader)',
     'V9A08_DeficitThreshold_IsStrictlyMoreThanFifteenPercentOfTheLeader'),
    ('N7', '落後判定的分母用自身分', LOGIC + 'CaptureMatchLogic.cs',
     '_tuning.RageDeficitDenominator * (leader - ownScore) > _tuning.RageDeficitNumerator * leader)',
     '_tuning.RageDeficitDenominator * (leader - ownScore) > _tuning.RageDeficitNumerator * ownScore)',
     'V9A08_DeficitThreshold_IsStrictlyMoreThanFifteenPercentOfTheLeader'),
    ('N8', '狂怒累加', LOGIC + 'CaptureMatchLogic.cs',
     '                _rageRemaining[side] = _tuning.RageSeconds;   // E15：刷新回 12，不累加\n',
     '                _rageRemaining[side] += _tuning.RageSeconds;   // E15：刷新回 12，不累加\n',
     'V9A09_RageLastsTwelveSeconds_RefreshesOnASecondCut_NotOnAnIslandFlip_AndClampsAtZero'),
    ('N9', '狂怒中再斷能不刷新', LOGIC + 'CaptureMatchLogic.cs',
     '                _rageRemaining[side] = _tuning.RageSeconds;   // E15：刷新回 12，不累加\n',
     '                if (_rageRemaining[side] <= 0f) _rageRemaining[side] = _tuning.RageSeconds;\n',
     'V9A09_RageLastsTwelveSeconds_RefreshesOnASecondCut_NotOnAnIslandFlip_AndClampsAtZero'),
    ('N10', '觸發當 tick 就倒數（②′ 搬到 ③b 之後）', LOGIC + 'CaptureMatchLogic.cs',
     '            TickRage(dt);\n\n            // ② 算雙方在哪個光圈（倒地方不算）\n            int blueCircle = _blueKnockedOut ? -1 : _spec.CircleAt(heroX, heroZ, _tuning.CircleRadius);\n            int redCircle = _redKnockedOut ? -1 : _spec.CircleAt(opponentX, opponentZ, _tuning.CircleRadius);\n\n            // 本 tick 翻下的塊（每方至多一塊）：各方只能翻自己所在的光圈，而雙方同圈是爭奪、不翻塊，\n            // 所以「該圈在 ③ 之後變成己方、③ 之前不是」就是本 tick 翻下的那塊。\n            int blueCircleOwnerBefore = blueCircle == -1 ? -1 : _ownership[blueCircle];\n            int redCircleOwnerBefore = redCircle == -1 ? -1 : _ownership[redCircle];\n\n            ProcessChannels(blueCircle, redCircle, dt);   // ③ 引導／爭奪／翻塊\n\n            int blueFlipped = blueCircle != -1 && blueCircleOwnerBefore != BlueFactionId && _ownership[blueCircle] == BlueFactionId ? blueCircle : -1;\n            int redFlipped = redCircle != -1 && redCircleOwnerBefore != RedFactionId && _ownership[redCircle] == RedFactionId ? redCircle : -1;\n            if (blueFlipped != -1 || redFlipped != -1) ProcessEncircle(blueFlipped, redFlipped);   // ③b 包夾 BFS 與狂怒判定\n',
     '\n            // ② 算雙方在哪個光圈（倒地方不算）\n            int blueCircle = _blueKnockedOut ? -1 : _spec.CircleAt(heroX, heroZ, _tuning.CircleRadius);\n            int redCircle = _redKnockedOut ? -1 : _spec.CircleAt(opponentX, opponentZ, _tuning.CircleRadius);\n\n            // 本 tick 翻下的塊（每方至多一塊）：各方只能翻自己所在的光圈，而雙方同圈是爭奪、不翻塊，\n            // 所以「該圈在 ③ 之後變成己方、③ 之前不是」就是本 tick 翻下的那塊。\n            int blueCircleOwnerBefore = blueCircle == -1 ? -1 : _ownership[blueCircle];\n            int redCircleOwnerBefore = redCircle == -1 ? -1 : _ownership[redCircle];\n\n            ProcessChannels(blueCircle, redCircle, dt);   // ③ 引導／爭奪／翻塊\n\n            int blueFlipped = blueCircle != -1 && blueCircleOwnerBefore != BlueFactionId && _ownership[blueCircle] == BlueFactionId ? blueCircle : -1;\n            int redFlipped = redCircle != -1 && redCircleOwnerBefore != RedFactionId && _ownership[redCircle] == RedFactionId ? redCircle : -1;\n            if (blueFlipped != -1 || redFlipped != -1) ProcessEncircle(blueFlipped, redFlipped);   // ③b 包夾 BFS 與狂怒判定\n            TickRage(dt);\n',
     'V9A09_RageLastsTwelveSeconds_RefreshesOnASecondCut_NotOnAnIslandFlip_AndClampsAtZero'),
    ('N11', '孤島翻塊也算斷能', LOGIC + 'CaptureMatchLogic.cs',
     '                    if (i != justFlipped) cutEvent = true;\n',
     '                    cutEvent = true;\n',
     'V9A13_BfsDoesNotTouchChannelState_AndAnIslandFlipIsNotACutEvent'),
    ('N12', '復活優先序依索引（選索引最小的仍持有母板塊）', LOGIC + 'CaptureMatchLogic.cs',
     ...
     '            int best = -1;\n            for (int rank = 0; rank < _spec.MotherCount(side); rank++)\n            {\n                int m = _spec.MotherTile(side, rank);\n                if (_ownership[m] == side && (best == -1 || m < _spec.MotherTile(side, best))) best = rank;\n            }\n            if (best != -1)\n            {\n                x = _spec.MotherRespawnX(side, best);\n                z = _spec.MotherRespawnZ(side, best);\n                return;\n            }\n',
     'V9A11_RespawnPriority_BlueThirteenTwelveFourteen_RedSevenEighteenEight'),
    ('N13', '復活地點在 ③ 之前判定（ProcessKnockouts 搬到 ProcessChannels 正前方）', LOGIC + 'CaptureMatchLogic.cs',
     '            int blueCircleOwnerBefore = blueCircle == -1 ? -1 : _ownership[blueCircle];\n            int redCircleOwnerBefore = redCircle == -1 ? -1 : _ownership[redCircle];\n\n            ProcessChannels(blueCircle, redCircle, dt);   // ③ 引導／爭奪／翻塊\n\n            int blueFlipped = blueCircle != -1 && blueCircleOwnerBefore != BlueFactionId && _ownership[blueCircle] == BlueFactionId ? blueCircle : -1;\n            int redFlipped = redCircle != -1 && redCircleOwnerBefore != RedFactionId && _ownership[redCircle] == RedFactionId ? redCircle : -1;\n            if (blueFlipped != -1 || redFlipped != -1) ProcessEncircle(blueFlipped, redFlipped);   // ③b 包夾 BFS 與狂怒判定\n\n            // ④ 必須排在 ③ 之後：復活地點看得到本 tick 的翻塊（E20；把它搬到 ③ 之前就是突變 N13）。\n            ProcessKnockouts(dt);   // ④ 倒地倒數與復活地點\n\n            ProcessScoring(dt);   // ⑤ 計分時鐘 ⑥ 勝負\n',
     '            int blueCircleOwnerBefore = blueCircle == -1 ? -1 : _ownership[blueCircle];\n            int redCircleOwnerBefore = redCircle == -1 ? -1 : _ownership[redCircle];\n\n            ProcessKnockouts(dt);   // ④ 倒地倒數與復活地點\n            ProcessChannels(blueCircle, redCircle, dt);   // ③ 引導／爭奪／翻塊\n\n            int blueFlipped = blueCircle != -1 && blueCircleOwnerBefore != BlueFactionId && _ownership[blueCircle] == BlueFactionId ? blueCircle : -1;\n            int redFlipped = redCircle != -1 && redCircleOwnerBefore != RedFactionId && _ownership[redCircle] == RedFactionId ? redCircle : -1;\n            if (blueFlipped != -1 || redFlipped != -1) ProcessEncircle(blueFlipped, redFlipped);   // ③b 包夾 BFS 與狂怒判定\n\n            // ④ 必須排在 ③ 之後：復活地點看得到本 tick 的翻塊（E20；把它搬到 ③ 之前就是突變 N13）。\n\n            ProcessScoring(dt);   // ⑤ 計分時鐘 ⑥ 勝負\n',
     'V9A10_AllMothersLost_NeutralizesEverything_AndRespawnGoesToTheEdge_IncludingOnTheSameTick'),
    ('N14', '相鄰表漏 4–13', LOGIC + 'CaptureBoardSpec.cs',
     '                new[] { 0, 3, 5, 12, 13, 14 },\n                new[] { 0, 4, 6, 14, 15, 16 },\n                new[] { 0, 1, 5, 16, 17, 18 },\n                new[] { 1, 8, 18 },\n                new[] { 1, 2, 7, 9 },\n                new[] { 2, 8, 10 },\n                new[] { 2, 3, 9, 11 },\n                new[] { 3, 10, 12 },\n                new[] { 3, 4, 11, 13 },\n                new[] { 4, 12, 14 },\n',
     '                new[] { 0, 3, 5, 12, 14 },\n                new[] { 0, 4, 6, 14, 15, 16 },\n                new[] { 0, 1, 5, 16, 17, 18 },\n                new[] { 1, 8, 18 },\n                new[] { 1, 2, 7, 9 },\n                new[] { 2, 8, 10 },\n                new[] { 2, 3, 9, 11 },\n                new[] { 3, 10, 12 },\n                new[] { 3, 4, 11, 13 },\n                new[] { 12, 14 },\n',
     'V9A03_Adjacency_MatchesTheFortyTwoEdgeTable'),
    ('N15', '進入 Ended 不清狂怒', LOGIC + 'CaptureMatchLogic.cs',
     '                    // E15：進入 Ended 時雙方狂怒清為 0。\n                    _rageRemaining[BlueFactionId] = 0f;\n                    _rageRemaining[RedFactionId] = 0f;\n',
     '',
     'V9A14_EndedClearsRage_AndTheNextMatchStartsAtZero'),
    ('N16', '比分表上限停在 1012', LOGIC + 'CaptureHudLabels.cs',
     '        public const int MaxScore = 1037;\n',
     '        public const int MaxScore = 1012;\n',
     'V9A16_ScoreTableTo1037_AndRageLabelsRoundUp'),
    ('N17', '7 塊夾具開啟包夾', LOGIC + 'CaptureBoardSpec.cs',
     '                false, false);\n',
     '                true, false);\n',
     'V9A17_SevenTileFixture_KeepsV080Rules_NoEncircleNoRage'),
    ('N18', '對手選點平手取索引大（19 塊多載的 < 改 <=）', LOGIC + 'CaptureOpponentPolicy.cs',
     '                if (best == -1 || distSq < bestDistSq)\n',
     '                if (best == -1 || distSq <= bestDistSq)\n',
     'V9A15_OpponentTargetSelection_NineteenTiles_SmallestIndexOnExactTie'),
]
