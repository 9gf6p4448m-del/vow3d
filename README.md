...
1. **0.05ms 格點阻擋網格與局部向量場 (Bitmask Grid Flow-Field)**：
   - **嚴禁全圖 NavMesh 動態重新烘焙！** 戰場底層採用 0.5 米精度的二維阻擋網格。石牆升起/破碎時，只標記其佔據的 4 個網格為 `Blocked`，尋路以局部向量場避障。運算耗時小於 0.05ms，保證 10 人激烈交火 120 FPS 穩如泰山！
2. **定點數確定性與 3 幀無感平滑校正 (Fixed-Point Math & 3-Frame Smoothing)**：
   - 物理與碰撞位移全面採用 **定點數 (Fixed-Point Math)**，徹底消除 iOS、Android 與 PC 浮點數精度差異引發的同步漂移。
   - 網路丟包時，客戶端以 **3 幀線性差值 (33ms Smoothing)** 柔和校正，絕不產生畫面抽搐拉扯（Anti-Rubber-banding）。
...
[Phase 2: 雙態符印、破牆校驗與三大基石元素Combo]
  ...
  ├─ 符印雙擊 0.05s 極速前置石牆
  ├─ 0.05ms 格點阻擋網格與局部向量場避障 (Bitmask Grid Flow-Field, 零全圖烘焙)
  ├─ 陣營校驗破牆 (砸敵牆得護盾) vs 友軍穿透掩體 (10發上限無衰減)
  └─ 三大高對比元素反應 (泥濘流沙 1.2s 禁位移、蒸氣霧受擊顯影、擴散火浪)
...
[Phase 4: 跨平台網絡同步、溫控與發行]
  ├─ 定點數物理 (Fixed-Point Math) 跨端絕對一致性
  ├─ 樂觀預測 (Client-Side Prediction) 與 3 幀平滑糾偏 (Anti-Rubber-banding)
